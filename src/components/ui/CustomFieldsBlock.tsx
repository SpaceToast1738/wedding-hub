...
// v1.22.0: shared custom-fields block for any entity (Guest / Supplier
// / Task). The C10/v1.15.0 original was Guest-coupled (imported the
// guests action directly); this version takes an `onSave(fieldId,
// rawValue)` callback so the parent decides which server action to
// call. Validation happens server-side regardless.
...
type Props = {
  ...
  canEdit: boolean;
  onSave: (fieldId: string, rawValue: string | null) => Promise<void>;
  /**
   * Optional title override (defaults to "Custom fields"). Useful when
   * a parent wants to differentiate ("Custom fields — supplier", etc).
   */
  title?: string;
};

export function CustomFieldsBlock({ fields, values, canEdit, onSave, title = "Custom fields" }: Props) {
