...
export function AddHouseholdToggle() {
  ...
  if (!open) {
    return <Button variant="primary" size="sm" onClick={() => setOpen(true)}>+ New household</Button>;
  }
  return (
    <div className="bg-surface border border-moss-100 rounded-md p-4 mb-4 shadow-sm">
      <h3 className="text-sm font-semibold text-ink-primary mb-3">New household</h3>
      <form
        action={(fd) => startTransition(async () => { await createHousehold(fd); setOpen(false); })}
        className="space-y-3"
      >
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <Input name="name" required autoFocus defaultValue="" placeholder="e.g. The Spencer Family" className="sm:col-span-2" />
          <select name="side" defaultValue="BOTH" className="text-sm bg-surface border border-border-soft rounded-sm px-2 py-1.5 text-ink-primary outline-none">
