...
export const proposeBookDresscodeUpdate: AiTool<typeof inputSchema> = {
  ...
  description:
    "Propose changes to a DRESS_CODE Wedding Book card — dress-code label, summary, longer body text, and guidance on colours, footwear, weather and accessories. Writes a proposal — does NOT change the card. Express only what changes; anything you don't name is preserved; you cannot see or change money, photos, or layout. bodyText is plain text — it REPLACES the card's whole body when set, so carry forward anything worth keeping (read it via read_book_card first).",
  inputSchema,
  progressLabel: "Proposing dress-code update…",
  definition: {
    name: "propose_book_dresscode_update",
    description:
      "Propose an update to a DRESS_CODE card. Writes a proposal — does not change the card directly. Express only what changes; omitted fields are preserved. bodyText is plain text and replaces the whole body when set — read the card first via read_book_card.",
    input_schema: {
      type: "object",
      properties: {
        subsectionId: {
        ...
        bodyText: {
          type: ["string", "null"],
          description:
            "Plain-text body — REPLACES the current body entirely when set. null clears.",
        },
        colourGuidance: {
          type: ["string", "null"],
