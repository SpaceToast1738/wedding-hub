...
**Design — one document, three renderers.** The Book has 13 card kinds, so per-kind "→ WhatsApp" and "→ print" serialisers would be 26 renderers that drift. Instead every kind builds one neutral `Doc` (title + blocks: headings, paragraphs, quotes, lists, label/value rows, tables — `src/lib/book-card-doc.ts`, pure) and the formats are tiny renderers over it: `docToWhatsApp` (`*bold*` headings, `•` / `1.` lists, `_italic_`, links as `text (url)`, marks hugging the text the way WhatsApp parses them), `docToPlainText`, and `DocView` for print. Allow-listed book HTML (TEXT and DRESS_CODE bodies) parses into real blocks via `htmlToBlocks` — tags gone, entities decoded, `<strong>/<em>/<a>` kept as marks; a legacy plain `body` becomes paragraphs. Structured kinds map naturally: MENU = a heading per course with options as bullets (dietary / veg-main / kids flags), SETUP / BAR / BUILD / LODGING = item bullets with location / supplier / state, STAY = label/value rows, WEDDING_PARTY = a members × items table (one line per member in text), FIELD = grouped label/value rows of the filled fields only. **Money never appears** (parity with `read_book_card`). The loader (`src/lib/core/book-export.ts`) mirrors `read_book_card`'s selects.

**Surface.** A **Share** button in every card's footer — for every viewer, not just editors, because the people who forward a brief are usually view-only — with *Copy for WhatsApp*, *Copy as plain text*, and *Print / save as PDF*. The last opens `/book/print/[subsectionId]`, a one-card page in the wedding theme that hides the app chrome under `@media print`, so the browser's Save-as-PDF is the PDF engine (no PDF library, same pattern as the catering brief and schedule). Gate: book VIEW plus the COUPLE_ONLY wall for non-couple users (`share-actions.ts`, kept out of the 3,000-line `book/actions.ts` because it is a read with a different gate).

**Verification.** typecheck ✅, 860 tests ✅ (13 new in `tests/unit/book-card-doc.test.ts`: inline marks + entity decoding, block parsing incl. legacy bodies, the exact ceremony-brief WhatsApp output, mark hugging, and MENU / SETUP / WEDDING_PARTY / FIELD / STAY docs), lint ✅, build ✅ (`/book/print/[subsectionId]` route present).
...
**propose_nudge_send (the headline).** Proposal-gated sending of the RSVP-chase / overdue-task nudge digest that `read_nudge_preview` previews — the most side-effectful proposal kind yet, because **Apply actually emails** the couple + planners. Triple-gated: ai_write EDIT + couple-tier + a new per-token flag **`McpToken.canProposeSend`** (default off, alongside canApply/canDismissOwn — the standing "sends are opt-in" invariant). The proposal payload **snapshots the recipient list + item count** so the `/ai` reviewer sees exactly who gets emailed; the apply path (`nudge.send` in execute.ts → `sendDigestCore`) **re-derives eligibility from live data at send time** so nobody who has since responded is chased and the 7-day cooldown always holds. Idempotent by construction: the atomic PENDING→APPLIED claim makes re-applying the same proposal a no-op, and the core sends the email BEFORE the best-effort `lastNudgedAt` stamp (a post-send bookkeeping hiccup is logged, never thrown, so it can't trigger a rollback-and-resend). The send machinery (digest builders, recipient lookup, eligibility→send→stamp→audit) moved from `settings/nudge-actions.ts` into the session-free `src/lib/core/nudge.ts`; the human `sendDigestEmail` action is now a thin gate-then-delegate.

**Table rename + shape on propose_seating_table_update.** `name` + `shape` are now editable (the "table names or numbers" decision needed a write path). New AI-apply-only cores `updateTableNameCore` / `updateTableShapeCore` (no human form renames a table today — same posture as v2.9.0's `updateSupplierContactCore`). Shape never changes the seat count, so it carries no eviction risk; a capacity change in the same proposal still runs the existing shrink-repack guard.

**read_tasks pagination + assignee filter.** `offset` (mirroring read_seating/read_budget's offset/limit → `page.nextOffset`, with an `id` tiebreaker so pages are stable) plus an `assignee` filter (user id exact OR case-insensitive name/first-name substring). The hub's 90+ open tasks are now fully enumerable past the 50-cap.

**Wedding-settings surface.** `read_settings` (couple-only) returns the wedding date, ceremony time, venue, couple names, the AI monthly cap + its effective value + source, whether an API key is set, and the AI/MCP kill-switch states — so the wedding-date discrepancy that was only visible via read_stats is now first-class (and now that v2.9.1 has corrected the canonical date to 24 Sept, this is the tool to confirm it). `propose_settings_update` writes back **only** `weddingDate` and `aiMonthlyCapPence` (venue/couple/names/API key stay a human-only form edit), couple-only at apply, via the new session-free `updateWeddingSettingsPartialCore`. The tool tells the agent a date change ripples into schedule/stays/payments.

**Batchables shipped:** budget category rename (`propose_budget_category_update` → `renameCategoryCore`, human `renameCategory` now delegates); moving a line between categories (opt-in `categoryId` on `propose_budget_line_update`, target-existence validated at apply — no silent relocation); plan-level seating notes/checklist write (`propose_seating_plan_update` → the `updateSeatingNotesCore` / `updateSeatingChecklistCore` extractions the human seating actions now delegate to).

**Deliberately left out:** wedding-party member/item **removal** tools. Add exists; remove doesn't. Removals need destructive-snapshot treatment (cascade over the member's cells) + new cores, which is a bigger/riskier slice than the rest of this tier — deferred to keep the diff focused. Everything else on the brief shipped.

Four new proposal kinds (`nudge.send`, `settings.update`, `budget.category.update`, `seating.plan.update`); five new tools (read_settings + four propose tools) plus enhancements to three existing tools; `canProposeSend` threaded through the token model, `verifyMcpToken`, the route ctx, `toolDefinitions` (additive — propose_nudge_send lists only for canWrite+canProposeSend, never in the in-app chat), and the Settings token panel (third per-token checkbox + `setMcpTokenCanProposeSend` action, audited). Planner system prompt + PLANNER.md + docs/MCP.md updated. **Typecheck clean, 796 tests green (57 files — new: v2.9.2 schema fences; extended: registry-seam canProposeSend gating + read_settings, mcp-auth flag passthrough), lint clean, `next build` verified.** `canProposeSend` defaults off, so shipping this changes nothing about live behaviour until Jamie enables a token.
...
Four capability gaps closed plus one standing rule, built on the v2.8.x propose→apply machinery. One additive migration (`20260720000000_mcp_token_can_dismiss_own` — hand-authored SQL following the v2.8.0 precedent since no local dev DB exists on this machine; DDL verified byte-identical against `prisma migrate diff`).

**propose_supplier_contact_update.** Patches an existing contact person (name/role/email/phone/primary) by contactId — previously only add/remove existed and the remove tool literally told the agent to delete-and-recreate. Deltas: omitted keeps, null clears; `primary:true` runs the same unmark-siblings swap as contact-add, in one transaction (`updateSupplierContactCore` in src/lib/core/suppliers.ts — AI-apply-only today, ready for a future edit form). `read_suppliers` now returns a full `contacts` array (id/name/role/email/phone/primary) as the id currency; the old `primaryContact` field is kept byte-compatible.

**propose_book_section_update.** Renames a section's title/subtitle. **The slug is never re-derived** — it stays stable across renames (the v1.94.0 human rule), so /book links and sectionSlug references survive; a test pins that the update never writes a slug. Write half extracted to `updateBookSectionCore` (human `updateBookSection` delegates, byte-identical); apply path carries the same COUPLE_ONLY wall as section delete.

**propose_file_upload.** Proposal-gated upload (folder, filename, MIME, base64, EVERYONE|COUPLE_ONLY). Bytes are NEVER stored in the proposal row: the tool decodes (strict base64 validation), enforces the src/lib/uploads.ts MIME allowlist and a **10 MB cap** (below the human 25 MB), and stages the file under UPLOADS_DIR as `pending-<32hex>.<ext>` (src/lib/ai/uploads-staging.ts). Apply renames the stage to a real stored name then creates the File row (rename-back on insert failure keeps the proposal retryable); Dismiss — and supersede — deletes the staged bytes; stages older than 7 days are swept whenever a new stage is created. The strict staged-name pattern makes payload tampering unable to point filesystem calls outside the uploads dir. Until applied, the file is invisible everywhere (no File row exists). **Transport caps raised in lockstep:** MCP route body cap 256 KB → 16 MB, Caddy `:8090` `request_body` 2 MB → 16 MB — **host Caddyfile must be synced + caddy restarted at deploy** or >2 MB uploads die at the proxy.

**Proposal supersede.** Every `propose_*` call accepts an optional `supersedesProposalId` (route-level, like `batchKey` — non-strict Zod drops it from tool inputs). Once the new proposal exists, the old one — still PENDING and created by the same token's user — is atomically dismissed with a `superseded by <id>` metadata note (audited `ai.proposal.superseded`); the outcome rides an extra JSON line in the tool result. No new review status: DISMISSED + note keeps the /ai panel untouched (Jamie's brief allowed this explicitly). "Same token" is approximated as same user — proposals don't record the minting token.

**Stretch shipped: `McpToken.canDismissOwn`.** Narrower than canApply: lists ONLY `dismiss_proposals`, and the handler pre-filters to rows the token's user created (foreign/missing ids get the non-leaking "Proposal not found." — strict even for couple-tier users, since small blast radius is the point). Settings gains a second per-token checkbox + `setMcpTokenCanDismissOwn` action, both audited. **canApply keeps defaulting false** (standing invariant), and so does this flag.

**Consistency rule (Jamie's add, 2026-07-20).** The MCP `initialize` instructions now tell every connected client to ripple-check updates — propose the consistency fixes in the same batch (shared batchKey) or say what was left stale — with a compact date/booking/RSVP example inline. The full ripple map lives in a new seventh planner prompt, **`consistency_check`** (optional `change` argument; falls back to a whole-hub audit), and PLANNER.md gains the matching working-rhythm step.

Planner system prompt updated for the three new tools. Docs: MCP.md (token-flag table, supersede/staged-upload/consistency behaviour notes), PLANNER.md. Typecheck clean, 781 tests green (56 files — new suites: staging lifecycle incl. TTL sweep, apply-files ordering + rollback, supersede races/ownership, dismiss-own gating, v2.9 schema fences, section-rename slug stability), lint clean, `next build` verified.
...
The blocker was macvlan isolation, confirmed live: the Unraid host cannot reach caddy's br0 address `192.168.50.25` at all, so tailnet traffic routed through Tower (and SSH tunnels originating on it) could never reach the `:8090` listener. First attempt — a `ports:` mapping on the caddy service — failed live in an instructive way: Docker registers the binding but **silently skips publishing when the container's primary network is macvlan** (`HostConfig.PortBindings` populated, `NetworkSettings.Ports` null, nothing listening). Fix: a new `mcp-proxy` service (`alpine/socat`, bridge-only, so its publish works normally) relays Tower's `:8090` to `caddy:8090` by service name, and the route's `MCP_LAN_HOST` Host-allowlist is now comma-separated so `192.168.50.110:8090` (LAN via Tower) and `100.79.99.19:8090` (tailnet) are admitted alongside the original listener address.
...
**Favicon + PWA assets** — a font-free geometric mark (two interlocking rings, moss-700 on canvas) so there's no Fraunces-embedding dependency for a 16px glyph:
- `src/app/icon.png` (32×32, browser tab favicon) and `src/app/apple-icon.png` (180×180, iOS home-screen) — both static PNGs, Next's file-based icon convention auto-wires the `<link>` tags.
- `public/icon-192.png` / `public/icon-512.png` + `src/app/manifest.ts` — "Add to Home Screen" now shows the branded mark and app name (`display: standalone`, moss-700 theme colour). No service worker added — one was deliberately removed earlier (`ServiceWorkerCleanup`) after stale-cache bugs, and a manifest alone doesn't need one.
- `viewport.themeColor` added to `layout.tsx` (static `#3F4F30`, not `prefers-color-scheme`-driven — dark mode here is a stored user preference via `DarkModeScript`, not an OS media query, so a media-query theme-color would drift from what's on screen).
- **Build trap hit and fixed**: the obvious approach — `next/og`'s `ImageResponse` rendering the mark dynamically via `icon.tsx`/`apple-icon.tsx` — built cleanly on `tsc` but failed `next build` with `TypeError: Invalid URL` while prerendering. Root cause: Next 15.0.4's bundled `@vercel/og` loads its default font via module-scope `fs.readFileSync(fileURLToPath(join(import.meta.url, "../noto-sans-v27-latin-regular.ttf")))` — `path.join` (both `win32` and `posix`) mangles a `file://` URL string, throwing before any of our code runs, regardless of whether custom fonts are supplied. Reproduced directly with `path.win32.join`/`path.posix.join` against a real `file://` string to confirm this fails identically on the Linux CI target, not just this Windows checkout — so this wasn't a "works in CI" situation to gamble on. Fix: dropped `next/og` entirely for icons, rendered the mark to raw SVG and rasterized with `sharp` (already resolved in `node_modules` as a Next.js optional dep) as a one-off script, committing the resulting static PNGs. Zero runtime dependency on `sharp` in production.

**Icon-system audit findings not acted on (recommendations, not fixes — need a style call, not a bug fix):** the nav's `✨` for AI is the only colour glyph in an otherwise-monochrome 16-icon set, but it's intentionally consistent with 8 other AI-CTA `✨` prefixes elsewhere, so left alone. ~24 scattered colour emoji (📎🔒✉☎🤝💬👥📍⚠) render in the OS's fixed emoji palette regardless of light/dark mode — cosmetically fine for a 5-person private app but would need a real icon library (none currently installed — confirmed zero `lucide`/`heroicons`/`react-icons` dependency) to theme consistently. Three parallel entity/status-glyph conventions (`RecentActivityFeed`'s `ENTITY_BADGE` map, seating's RSVP-dot triplet, the reorder/edit glyph family) never converged into one system. None of this blocks anything; flagged for whoever next has an opinion on it.

**Quick a11y win while in the icon code** — 6 bare-glyph reorder buttons (▲▼) across `BookWeddingPartyCard.tsx` (×2 sites), `CeremonyClient.tsx`, `GuestGroupsBlock.tsx`, `NavTagsBlock.tsx`, `PermissionGroupsBlock.tsx` had a `title` tooltip but no `aria-label` — screen readers announced them as unlabelled buttons in a list where every row's identical glyph made position the only way to tell rows apart. Added per-item `aria-label`s (e.g. `` `Move ${name} up` ``). Several sibling files already had this fixed in the v2.5.0 pass and were left alone.

637 tests green (unchanged — no business logic touched), typecheck clean, lint clean, full local `next build` verified after the ImageResponse→sharp swap (exit 0).

### 2026-07-05 · v2.5.0 — Full design/UX/accessibility pass (all 10 areas + design system)

User: "perform a full critique of the design and UX... it must be user & adhd friendly," then "can you fix everything on the list? Continue until the list is empty." Built as a multi-phase orchestration: 11 review agents produced ~90 findings across every page + the design system (with computed WCAG contrast ratios, not estimates), a shared primitives layer got fixed first, then 10 area agents fixed their findings in parallel, then a 27-agent adversarial review (9 finders → 18 raw findings → per-finding refutation → 17 confirmed) ran before ship. **All 17 confirmed findings fixed pre-commit.** No migrations.

**Design-system foundation (fixed once, repairs hundreds of call sites):**
- `ink-tertiary` darkened both modes (was failing AA at 3.6–3.8:1 everywhere it's used — form labels, metadata, due dates); `marigold-700` and `info` darkened similarly (warn-toast text, PENDING/LEAD pills, links).
- New `on-moss`/`on-danger` text tokens: dark mode flips the moss/danger scales light, so hardcoded `text-white` on a moss/danger fill was failing AA (as low as 1.65:1) — swept across **23 call sites app-wide**, not just the two the review flagged.
- `html { font-size: 100% }` / `body { font-size: 0.875rem }` split — the old combined `14px` rule silently compressed every rem-based Tailwind utility by 12.5% AND overrode the user's browser font-size preference; utilities now render at their nominal size again.
- `ConfirmDialog`/`AddNewModal`: removed an `aria-hidden` on the wrapper that was hiding the ENTIRE dialog (role, title, buttons) from screen readers on every one of the ~40 confirm-dialog call sites and every Add-modal in the app; added focus traps + focus restore to both.
- `Button`/`Tag`: 40px mobile touch floor baked into the primitive (`min-h-[40px] sm:min-h-0`) instead of per-site patches.
- `Toaster`: errors now persist until dismissed (were auto-dismissing in 6s — the only failure-reporting channel for 40+ server actions); every toast gets a click-to-dismiss × and pauses its timer on hover/focus.
- `Input` gained an optional `label` prop auto-wiring `htmlFor`/`id` via `useId()` — the app had ~147 sibling `<label>` blocks with only 4 `htmlFor` pairings (screen readers announced "edit text, blank").
- `StatusPill`/`Avatar`: hardcoded hexes promoted to tokens (`marigold-200`, `info-bg`, `info-border`, `avatar-accent`), each with a dark-mode value.

**Per-area fixes (10 parallel agents, disjoint files) — one item per area, see git history for the full ~90-item list:**
- **Today/Glance/Day-of**: day-of timeline now resolves modern `attendeeRefs` (was only reading legacy `attendeeIds`, silently blank for any group-scoped event); shared `priorityBarColour()` helper so Today and Glance stop disagreeing on what color "urgent" is; "My next tasks" no longer counts the whole project's open tasks as the denominator.
- **Tasks/Questions**: mobile list rows now show due date/assignee/status (were hidden entirely below 640px); overdue tasks render in danger color (were the lowest-contrast token in the palette); the done-circle is a straight DONE↔OPEN toggle instead of silently cycling to IN_PROGRESS while claiming "Mark done"; dead Category UI removed; new `TaskToolsMenu` collapses Gap analysis/Suggest due dates/Import CSV behind one trigger so "+ New task" reads as the obvious primary action.
- **Schedule**: timeline Edit/Delete fixed to the mobile-safe hover pattern (were touch-invisible); every mutation now toasts; a timed event with no start time no longer silently saves at midnight.
- **Suppliers**: a genuine money leak fixed — payment amounts and DIY budget-line estimates rendered on the detail page with no `showMoney` gate; all touch-invisible delete controls fixed; the detail page can now edit the supplier directly (status + full form) instead of being read-only.
- **Guests/Seating**: there was no way to add a single guest from the UI at all (only CSV import or the AI) — added a real Add-guest flow; seat-dot dragging on touch could silently unseat a guest via ordinary finger jitter — gated on the same fine-pointer check as table dragging.
- **Songs/DIY**: guest-request triage was a dead end (the UI told users to "add the ones you want" with no control that could do it) — added the missing add-to-playlist/dismiss actions; the DO-NOT-PLAY block list is now unmistakable (was ~95% visually identical to a normal playlist).
- **Wedding Book**: wedding-party matrix edit controls fixed to always-visible-while-editing (were touch-invisible even in the card's own edit mode); all 13 card kinds get a per-kind accent color for at-a-glance recognition.
- **Budget/Payments**: overdue payments now have real salience — `OVERDUE` was a manual status nobody set, so a payment three weeks late rendered identically to one due next month; CANCELLED payments no longer inflate the Outstanding total; mobile card-layout fallback replaces the horizontal-scroll tables.
- **Files/Settings/Auth**: file row actions fixed to the mobile-safe pattern; sign-in/verify forms now show pending state (were inviting double-submits on magic-link sends).
- **Shell/AI**: `/ai` — where every pending proposal lives — was in NO navigation surface at all; added a nav entry with a pending-count badge. QuickCapture was keyboard-shortcut-only, unreachable on touch; restored the prototype's floating "+" button. The closed chat panel stayed in the keyboard tab order despite `aria-hidden`.

**Adversarial review caught (all fixed):** a files batch-upload "atomic rollback" that only rolled back disk bytes, not committed DB rows, on a mid-batch failure; a genuine regression where the seating touch-drag fix also silently removed the desktop mouse-drag "drop off the table to unseat" gesture (restored, fine-pointer-only); the new floating AI-chat trigger was CSS-over-constrained so `sm:right-6` never actually took effect and the button stayed pinned bottom-left at every viewport; two check-then-act races in the new song-request triage actions (fixed with atomic `updateMany` claims); a payment manually marked OVERDUE didn't get the danger-colored due-date cell the derived check gives everything else; a wedding-book shot row could stay open in its edit form after the card itself returned to view mode; `marigold-200` had no dark-mode border override; a shared `CustomFieldsBlock` component (used by five different pages) was the one place the mobile-safe hover-gate sweep missed entirely.

637 tests green (unchanged — this pass touched UI/UX, not business logic), typecheck clean, lint clean, full local `next build` exit 0 (verified twice — once caught a missing prop destructure in `BudgetClient.tsx` and a second "use server" non-async export in `songs/actions.ts`, same class of bug as v2.4.0's build failure, fixed the same way: moved the constant to a plain module).
